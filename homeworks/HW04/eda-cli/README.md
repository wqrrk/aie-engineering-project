...
### 5. `POST /quality-flags-from-csv` – полный набор флагов качества (HW03)

Эндпоинт принимает CSV-файл и возвращает **полный набор флагов качества**, рассчитанных в EDA-ядре (`compute_quality_flags`), включая пользовательские эвристики, реализованные в рамках Семинара 03.

В отличие от эндпоинта `/quality-from-csv`, данный метод **не агрегирует результат в одно решение**, а возвращает детальную информацию о проблемах датасета.
...
### 6. `POST /summary-from-csv` – JSON-сводка датасета (как в CLI S03)

Эндпоинт является HTTP-обёрткой над JSON-сводкой, реализованной в CLI-версии проекта (Семинар 03).

Позволяет получить **структурированную сводку датасета** без генерации файлов отчёта.

В ответе содержится:
- размеры датасета;
- интегральная оценка качества данных;
- максимальная доля пропусков;
- списки проблемных колонок (пропуски, константы, дубликаты идентификаторов, большое количество нулевых значений).
...
```json
{
  ...
  "n_cols": 12,
  "quality_score": 0.78,
  "max_missing_share": 0.42,
  "problem_columns": {
    "missing_cols": ["age", "salary"],
    "constant_cols": ["country_code"],
    "id_duplicates": ["user_id"],
    "many_zeros": ["balance"]
  }
}
``` 
...
В HTTP-сервисе реализовано **структурированное логирование** запросов в формате JSON.

Для каждого HTTP-запроса в файл `logs/api.log` записывается одна строка лога со полями `endpoint` и необходимыми, переданными в зависимости от конкретного endpoint:
